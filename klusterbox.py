    ...
    if len(results) == 0:
        ...
        for record in results:
            ...
            rename_button[f]['command'] = \
                lambda frame = frame[f], \
                       tacs = record[0], \
                       kb = record[1], \
                       newname = si_newname[f], \
                       button = rename_button[f]: \
    ...
    if len(tacs_list) < 1 and len(new_carrier) < 1 and len(check_these) < 1:  # all tacs list resolved/ nothing to check
        ...
        for t_name in tacs_list:
            ...
            Label(FF, text=t_name[1] + ", " + t_name[2], anchor="w", width=15, fg=color)\
                .grid(row=c, column=1, sticky="w")  # name
            Label(FF, text=t_name[3], anchor="w", width=10, fg=color)\
                .grid(row=c, column=2, sticky="w")  # assignment
...
def auto_indexer_6(frame, file_path):  # identify and remove any carriers in the carrier
    ...
    for name in ex_carrier:
        ...
        new_station[c].set("out of station")
...
def carrier_list_cleaning(frame):  # cleans the database of duplicate records
    ...
    if ok:
        ...
        frame.destroy()
        main_frame()
    if not ok: messagebox.showinfo("Database Maintenance",
...
def spreadsheet(list_carrier, r_rings):
    ...
    for day in dates:
        ...
        formula = "=SUM(%s!I8:I%s)" % (day_of_week[i], cell)
        ...
        formula = "=SUM(%s!K8:K%s)" % (day_of_week[i], cell)
        ws_list[i]['K' + str(oi)] = formula  # OT off route
        ws_list[i]['K' + str(oi)].style = calcs
        ws_list[i]['K' + str(oi)].number_format = "#,###.00;[RED]-#,###.00"
        nl_totals = oi
        ...
        ws_list[i]['J' + str(oi)].style = col_header
        formula = "=SUM(%s!K%s:K%s)" % (day_of_week[i], top_cell, cell)
        ws_list[i]['K' + str(oi)] = formula  # OT off route
        ws_list[i]['K' + str(oi)].style = calcs
        ws_list[i]['K' + str(oi)].number_format = "#,###.00;[RED]-#,###.00"
        ...
        ws_list[i]['D' + str(oi)].style = col_header
        formula = "=SUM(%s!E%s:E%s)" % (day_of_week[i], top_cell, cell)
        otdl_total = oi
        ws_list[i]['E' + str(oi)] = formula  # availability to 10
        ws_list[i]['E' + str(oi)].style = calcs
        ws_list[i]['E' + str(oi)].number_format = "#,###.00;[RED]-#,###.00"
        formula = "=SUM(%s!F%s:F%s)" % (day_of_week[i], top_cell, cell)
        ws_list[i]['F' + str(oi)] = formula  # availability to 12
        ws_list[i]['F' + str(oi)].style = calcs
        ws_list[i]['F' + str(oi)].number_format = "#,###.00;[RED]-#,###.00"
        oi += 1
...
def edit_carrier(e_name):
    ...
    Button(delete_frame, text = "Delete", width=15,
           ...
           command = lambda: purge_carrier(switchF3, e_name)).grid(row=3, column=0, sticky=W)
