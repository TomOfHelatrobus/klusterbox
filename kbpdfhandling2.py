    ...
    class StartUp:
        ...
        def read_from_txt(self):
            ...
            if not messagebox.askyesno("PDF Converter", msg, parent=self.parent.frame):
                try:
                    self.parent.file_path = filedialog.askopenfilename(initialdir=dir_filedialog(),
        ...
        def create_raw_report(self):
            ...
            try:
                ...
                msg = "Klusterbox PDF Converter has encountered characters which can not be translated.\n\n" \
                      "Shutting down."
        ...
        def create_new_file(self):
            ...
            with open(self.parent.new_file_path, 'w') as writeFile:
                self.parent.writer = csv.writer(writeFile, dialect='myDialect')
                self.parent.writer.writerow(line)
            ...
            with open(self.parent.new_file_path, 'a') as writeFile:
                self.parent.writer = csv.writer(writeFile, dialect='myDialect')
                self.parent.writer.writerow(line)
    ...
    class PageAnalysis:
        ...
        def anaylse_pages(self):
            ...
            for page in self.parent.pages:
                ...
                self.parent.page_num += 1
            self.parent.endtime = time.time()
        ...
        def analyse_page(self, page):
            ...
            if not self.get_payperiod(page):  # find the pay period
                return
            if self.parent.saved_pp != self.parent.yyppwk:
            ...
            if self.parent.gen_error_report:
                ...
                self.parent.kbpc_rpt.write(datainput)
            self.parent.GroupAnalysis(self.parent).run(page)
        ...
        def new_csv(self):
            ...
            self.create_new_file()  # create the headers for the new file with self.create_new_file()
            # add shortened file name to an array for completion messagebox
            self.parent.multi_csv.append(self.get_shortname())
            ...
            for co in self.parent.csv_output:  # for each time in the array, write a line
                for array in co:
                    ...
                    if len(array) == 2:  # if the line comes from base/time data
                        ...
                        with open(self.parent.new_file_path, 'a') as writeFile:  # add the line to the csv file
                            self.parent.writer = csv.writer(writeFile, dialect='myDialect')
