...
class PdfConverter:
    ...
    def __init__(self):
        ...
        self.pages = None  # the documemnt is split into pages.
    ...
    def run(self, frame):
        ...
        if not self.validtext:  # if validate text has not been run in pdf to text.
            if not self.validate_text(self.pages[0]):  # search for a station name to make sure the doc is an ee report
                return
        if not self.get_station():  # find the station
            return
        # if not self.get_payperiod():  # find the pay period, get this as an array since some rprts have multiple
        #     return
        ...
        self.pb.max_count(len(self.pages) - 1)  # set length of progress bar
        self.pb.start_up()  # start the progress bar
        self.pbi = 1
        self.pb.move_count(self.pbi)  # increment progress bar
        self.pb.change_text("This process is underway. Please wait for results.")
        self.anaylse_pages()  # go page by page for analysis.
        self.pb.stop()
    ...
    def read_from_txt(self):
        ...
        msg = "The \"Read from text file\" option is selected for the pdf converter. This is an advanced setting " \
              "used for debugging and development. Go to \"Management > Pdf Converter Settings\" to disable it. \n\n" \
              "Select \"Yes\" to read from a text file generated by the pdf converter (this be a text file with a " \
              "suffix of \"_raw_kbpc.txt\").\n\n" \
              "Select \"No\" to read from a pdf file. Extra time will be needed to process the pdf file. "
        if not messagebox.askyesno("PDF Converter", msg, parent=self.frame):
            try:
            ...
            if not messagebox.askokcancel("PDF Converter", "This process will take several minutes. "
                ...
                if self.text == "Fail":  # pdf to text returns "Fail" if the process was unsucessful.
                    return False
            ...
            if self.validate_text(page[0]):
                ...
                if i < 1:
                    msg = "PDF Conversion has failed and will not generate a file.  \n\n " \
                          "We will try again."
                    result = messagebox.askokcancel("Klusterbox PDF Converter", msg, parent=self.frame)
                    if not result:
                        return text
                    ...
                    messagebox.showerror("Klusterbox PDF Converter",
                                         ...
                                         "information",
                                         parent=self.frame)
        self.validtext = True  # shows the test does not need to be repeated.
        return text
    ...
    @staticmethod
    def pdf_converter_path_generator(filepath, add_on, extension):
        """ generate csv file name and path """
    ...
    def create_raw_report(self):
        ...
        # generate csv file name and path
        self.kbpc_raw_rpt_file_path = self.pdf_converter_path_generator(self.file_path, "_raw_kbpc", ".txt")
    ...
    def split_document(self):
        ...
        self.pages = self.text.split("")  # split the document into pages
    ...
    def get_station(self):
        ...
        try:  # if the page has no station information, then break the loop.
            result = re.search("Restricted USPS T&A Information(.*)Employee Everything Report",
                               self.pages[0], re.DOTALL)
            self.station = result.group(1).strip()
            self.station = self.station.split('\n')[0]
            if not self.station:
                result = re.search("Employee Everything Report(.*)Weekly", self.pages[0], re.DOTALL)
                self.station = result.group(1).strip()
                self.station = self.station.split('\n')[0]
    ...
    def get_payperiod(self, field):
        ...
        if self.find_pp_search(start, end, field):
            return True
    ...
    def find_pp_search(self, start, end, field):
        """ look for the pay period using re.search.  """
        try:
            result = re.search('%s(.*)%s' % (start, end), field).group(1)
        except AttributeError:
            return False
        ...
        if re.match(r'[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9]', result):  # if the format is correct
            self.yyppwk = result  # save the result at the pay period
            return True
        return False

    def find_pp_findall(self, field):
        """ look for the pay period using re"""
        try:
            result = re.findall(r'[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9]', field)
            self.yyppwk = result[-1]  # select the last one
        except (AttributeError, IndexError):
            return False
        return True

    def anaylse_pages(self):
    ...
    def analyse_page(self, page):
        """ anaylse individual pages sent by anaylse pages. """
        if not self.get_payperiod(page):  # find the pay period
            return
        if self.saved_pp != self.yyppwk:
            if self.saved_pp:
                self.new_csv()  # create a new file path and create a new csv file.
            self.set_saved_pp()  # set/update the dates if the pay period is changed
        if self.gen_error_report:
            self.kbpc_rpt.write(
                "\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
                "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n")
        if page[0:6] == "Report" or page[0:6] == "":
            pass
        else:
            if self.gen_error_report:
    ...
    def new_csv(self):
        ...
        # update the self.new_file_path
        print(self.new_file_path)
        # create the headers for the new file with self.create_new_file()
    ...
    def anaylse_e(self, c):
        ...
        for e in d:
            ...
            if e == "Employee ID":
                ...
                self.eid_count = 0  # reset eid count to zero
    ...
    def find_franklin(self, e):
        ...
        if len(self.franklin_array) > 0 and re.match(r"[0-1][0-9]\/[0-3][0-9]$", e):  # if franklin array and date
    ...
    def bind_underscores(self):
        ...
        if len(self.daily_underscoreslash) > 0:
    ...
    def write_primeinfo(self):
        ...
        self.prime_info = [self.yyppwk.replace("-", ""), '"{}"'.format("000000"), '"{}"'.format(self.station),
    ...
    def init_csvdayarray(self):
        ...
        self.csv_thr = []
        self.csv_fri = []
        self.csv_output = [self.csv_sat, self.csv_sun, self.csv_mon, self.csv_tue,
                           self.csv_wed, self.csv_thr, self.csv_fri]
    ...
    def order_founddays(self):
        """ reorder the found days to ensure the correct order """
        if len(self.found_days) > 0:  # write out found days
            # reorder the found days to ensure the correct order
            self.found_days = self.pdf_converter_reorder_founddays()
            if self.gen_error_report:
    ...
    def csvoutput_time(self):
        ...
        for co in self.csv_output:  # for each time in the array, write a line
            for array in co:
                if self.gen_error_report:
                    datainput = "{}\n".format(array)
                    self.kbpc_rpt.write(datainput)
                ...
                if len(array) == 2:  # if the line comes from base/time data
                    ...
                    whole_line = self.prime_info + add_this
    ...
    def csvoutput_base(self):
        ...
        if len(self.jobs) > 0:
            for i in range(len(self.jobs)):
                ...
                whole_line = self.prime_info + base_line
                with open(self.new_file_path, 'a') as writeFile:
                    self.writer = csv.writer(writeFile, dialect='myDialect')
                    self.writer.writerow(whole_line)
    ...
    def handle_underscoreslash(self):
        ...
        self.csv_thr = []
        self.csv_fri = []
        self.csv_output = [self.csv_sat, self.csv_sun, self.csv_mon, self.csv_tue, self.csv_wed,
                           self.csv_thr, self.csv_fri]
        # reorder the found days to ensure the correct order
        self.found_days = self.pdf_converter_reorder_founddays()
        # fix problem with miscounted base times
        high_array = []
        ...
        for i in range(7):
            ...
            for da in self.daily_array:
                if da[2] == self.pp_days[i].strftime("%m/%d"):
                    self.csv_output[i].append(da)
        for co in self.csv_output:  # for each time in the array, printe a line
            for array in co:
                if self.gen_error_report:
                    datainput = "{}\n".format(str(array))
                    self.kbpc_rpt.write(datainput)
